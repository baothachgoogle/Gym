...
# ---------- Newly introduced util functions ---------- #
def convert_trajectory_to_output_items(
    ...
    """
    output_items = []
    for msg_idx, raw_message_model in enumerate(trajectory):
        raw_message = raw_message_model.model_dump()
        role = raw_message.get("role") or ""
        content = raw_message.get("content") or ""

        if role in ["user", "system"]:  # type(item) = UserMessage or SystemMessage
            if content:
                output_items.append(
                    NeMoGymMessage(
        ...
        elif role == "tool":  # type(item) = ToolMessage
            # Tool response
            output_items.append(
                NeMoGymFunctionCallOutput(
        ...
        elif role == "assistant":  # type(item) = AssistantMessage
            ...
            if (
                ...
                and "message" in raw_message["raw_data"]
                and "provider_specific_fields" in raw_message["raw_data"]["message"]
            ):
                provider_fields = raw_message["raw_data"]["message"][
                    "provider_specific_fields"
                ]
                prompt_token_ids = provider_fields.get("prompt_token_ids", []) or []
                generation_token_ids = (
                    provider_fields.get("generation_token_ids", []) or []
                )
                generation_log_probs = (
                    provider_fields.get("generation_log_probs", []) or []
                )
            if prompt_token_ids:
                last_response_output_item = output_items[-1]
                train_cls = RESPONSES_TO_TRAIN[last_response_output_item.__class__]
                output_items[-1] = train_cls(
...
# ---------- Below are utils copied from tau2 bench codebase ---------- #
# ---------- https://github.com/sierra-research/tau2-bench   ---------- #
# The code is modified:
# 1) Override the save_to to None
# 2) Override the user response to "y" for auto-resume of existing simulation runs.
...
def run_domain(config: RunConfig) -> Results:
    ...
    save_to = None  # Comment this out to enable saving of simulation results.
...
def run_tasks(
    ...
    if save_to is not None:
        ...
        if save_to.exists():
            ...
            # Manually override to "y" for now to enable auto-resume.
            response = "y"
            if response != "y":
    ...
    def _run(task: Task, trial: int, seed: int, progress_str: str) -> SimulationRun:
        ...
        try:
            ...
            raise e  # TODO: Bao check whether to uncomment this
            # return None
            # return "meowbao"
        return simulation
